...
const { decode } = useDecoder({
    ...
    canUseUtf16: isNode(CURRENT_PLATFORM) || isBun(CURRENT_PLATFORM),
...
type SearchModule = {
    readonly memory: WebAssembly.Memory
    readonly tryFindEndOfString: (start: number) => number
}

type ParserOptions = {
    readonly maxWasmMemoryPages: number
    readonly initialWasmMemoryPages: number
    readonly canUseUtf16: boolean,
...
export function stringParser(options: ParserOptions) {
    const { initialWasmMemoryPages, maxWasmMemoryPages } = options
    ...
    const searchModule = wasmInstance<SearchModule>(new Uint8Array([]), memory)

    const tryFindEndOfString = searchModule?.tryFindEndOfString ?? ((): number => {
    ...
    const decodeFactory = () => {
        if (options.canUseUtf16) {
            ...
            if (module) {
                ...
                return (reader: JsonReader, i: number, end?: number) => {
...
function useDecoder(options: ParserOptions) {
    try {
        ...
        const module = instance.exports as ParseModule
