...
export function stringParser(options: ParserOptions) {
    ...
    const tryFindEndOfString = utf8Module?.try_find_end_of_string ?? ((start: number): number => {
        return 0
    })
        ...
        if (opt.canExtendToUtf16) {
            ...
            if (module) {
                ...
                return (reader: JsonReader, i: number, end: number) => {
                    ...
                    return {
                        type: COMPLETE,
                        value: utf16(b.length + 1, utf16_end),
                        nextIndex: index + 1
                    }
                }
            }
    ...
    const decodeUTF8Exact = decodeFactory({ ...options, canExtendToUtf16: false })
    const decodeUTF16Inexact = decodeFactory(options)
