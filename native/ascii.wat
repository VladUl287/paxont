(module
  ...
  (func $parse_ascii (export "parse_ascii") (param $i i32) (param $end i32) (param $target i32) (param $extend i32) (result i32 i32)
    (local $start i32)
      ...
      (loop $loop
        ...
        (if (local.tee $byte (i8x16.bitmask (i8x16.eq (local.get $data_vec) (local.get $quote_vec))))
          (then
            ...
            (if (i32.le_u (local.get $byte) (local.get $byte_count)) 
              (then (local.set $byte_count (local.get $byte)))
            )
            ...
            (return (i32.add (local.get $i) (local.get $byte_count)) (local.get $target))
          )
        )
      ...
      (loop $loop
        ...
        (if (i32.eq (local.get $byte) (i32.const 92))
          (then
            ...
            (return (call $parse_ascii (local.get $start) (local.get $end) (local.get $target) (i32.const 1)))
          )
        )

        (if (local.get $extend) 
          (then (local.set $target (call $store_32 (local.get $byte) (local.get $target))))
        )
              ...
              (if (i32.eq (local.get $byte) (i32.const 0x75))  ;; 'u'
                (then
                  ...
                  (if 
                      ...
                      (block $no_low_surrogate
                        ...
                        (if 
                          ...
                          (then
                            ...
                            (local.set $j (call $store_code_point (local.get $j) (local.get $code_point)))
                            (br $escape_done)
                          )
                          (else
                    ...
                    (else
                      (local.set $j (call $store_code_point (local.get $j) (local.get $hex_value)))
                    )
                  )
