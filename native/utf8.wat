(module
  (import "env" "memory" (memory 1 128))
  (import "utils" "find_quote" (func $find_quote (param $i i32) (param $start i32) (param $end i32) (result i32)))
  (import "ascii" "parse_ascii" (func $parse_ascii (param $i i32) (param $end i32) (param $target i32) (param $extend i32) (result i32 i32)))
  
  (global $ascii_only (mut i32) (i32.const 0))
  (global $dq_index (mut i32) (i32.const -1))
  (global $target (mut i32) (i32.const 0))

  (func (export "ascii_only") (result i32)
    (global.get $ascii_only))

  (func (export "dq_index") (result i32)
    (global.get $dq_index))

  (func (export "target") (result i32)
    (global.get $target))

  (func (export "utf8_to_utf8") (param $i i32) (param $end i32) (param $target i32) (result i32)
    (local $extend i32)
    (local $start i32)
    (local $target_temp i32)
    (local $mask i32)
    (local $temp i32)
    (local $temp_v128 v128)
    (local $quote_vec v128)
    (local $byte_mask i32)
    (local $trailing i32)
    (local $partialChar i32)
    (local $first i32)

    (local.set $start (local.get $i))

    (global.set $dq_index (i32.const -1))
    (global.set $ascii_only (i32.const 0))
    (global.set $target (local.get $target))

    (local.set $quote_vec (i8x16.splat (i32.const 34)))

    (call $parse_ascii (local.get $i) (local.get $end) (local.get $target) (i32.const 0))
    (local.set $target_temp)
    (local.set $i)

    (if (i32.eq (local.get $i) (i32.const -1))
      (then (return (i32.const -1)))
    )

    (if (i32.eq (local.get $i) (local.get $end))
      (then
        (global.set $ascii_only (i32.eq (local.get $target_temp) (local.get $target)))
        (global.set $target (local.get $target_temp))
        (return (local.get $i))
      )
    )

    (if (i32.eq (i32.load8_u (local.get $i)) (i32.const 34))
      (then 
        (if (i32.ge_s (local.tee $temp (call $find_quote (local.get $i) (local.get $start) (local.get $i))) (i32.const 0))
          (then
            (global.set $ascii_only (i32.eq (local.get $target_temp) (local.get $target)))
            (global.set $dq_index (local.get $temp))
            (global.set $target (local.get $target_temp))
            (return (local.get $temp)))
        )
      )
    )

    (if (i32.gt_u (local.get $target_temp) (local.get $target))
      (then
        (local.set $target (local.get $target_temp))
        (local.set $extend (i32.const 1))
      )
    )

    (block $block
      (loop $loop
        (if (i32.lt_u (i32.load8_u (local.get $i)) (i32.const 128)) 
          (then
            (call $parse_ascii (local.get $i) (local.get $end) (local.get $target) (local.get $extend))
            (local.set $target_temp)
            (local.set $i)
        
            (if (i32.eq (local.get $i) (i32.const -1))
              (then (return (i32.const -1)))
            )
        
            (if (i32.eq (local.get $i) (local.get $end))
              (then (return (local.get $i)))
            )

            (if (i32.eq (i32.load8_u (local.get $i)) (i32.const 34))
              (then 
                (if (i32.ge_s (local.tee $temp (call $find_quote (local.get $i) (local.get $start) (local.get $i))) (i32.const 0))
                  (then
                    (global.set $dq_index (local.get $temp))
                    (global.set $target (local.get $target_temp))
                    (return (local.get $temp)))
                )
              )
            )

            (if (i32.eqz (local.get $extend))
              (then
                (if (i32.gt_u (local.get $target_temp) (local.get $target))
                  (then
                    (local.set $target (local.get $target_temp))
                    (local.set $extend (i32.const 1))
                    (local.set $i (local.get $start))
                    (br $loop)
                  )
                )
              )
            )
          )
        )

        ;; two byte value
        (if (i32.lt_u (local.tee $temp (i32.load8_u (local.get $i))) (i32.const 224))
          (then
            (local.set $first (i32.const 1))
            (block $two_byte_block
              (loop $two_byte_loop 
                ;; i + 16 < len
                (if (i32.lt_u (i32.add (local.get $i) (i32.const 16)) (local.get $end))
                  (then 
                    (local.set $byte_mask
                      (i8x16.bitmask
                        (i16x8.eq
                          (v128.and
                            (local.tee $temp_v128 (v128.load (local.get $i))) 
                            (v128.const i16x8 0xC0E0 0xC0E0 0xC0E0 0xC0E0 0xC0E0 0xC0E0 0xC0E0 0xC0E0)
                          )
                          (v128.const i16x8 0x80C0 0x80C0 0x80C0 0x80C0 0x80C0 0x80C0 0x80C0 0x80C0)
                        )
                      )
                    )
  
                    (if (i32.eq (local.get $byte_mask) (i32.const 0xFFFF))
                      (then
                        (if (local.get $extend) 
                          (then
                            (v128.store (local.get $target) (local.get $temp_v128))
                            (local.set $target (i32.add (local.get $target) (i32.const 16)))
                          )
                        )
                        (local.set $i (i32.add (local.get $i) (i32.const 16)))
                        (local.set $first (i32.const 0))
                        (br $two_byte_loop)
                      )
                    )
  
                    (if (local.get $extend) 
                      (then
                        (v128.store (local.get $target) (local.get $temp_v128))
                        (local.set $target (i32.add (local.get $target) (local.get $trailing)))
                      )
                    )
                    (local.set $trailing (i32.ctz (i32.xor (local.get $byte_mask) (i32.const 0xFFFF))))
                    (local.set $i (i32.add (local.get $i) (local.get $trailing)))
                    (br $loop)
                  ))
  
                ;; i + 4 < len
                (if (i32.lt_u (i32.add (local.get $i) (i32.const 4)) (local.get $end))
                  (then 
                    (local.set $mask (i32.load offset=0 align=1 (local.get $i)))

                    (i32.ne 
                      (i32.and (i32.sub (local.get $mask) (i32.const 32960)) (i32.const 49376))
                      (i32.const 0)
                    )
                    (if 
                      (then
                        (br_if $two_byte_block (local.get $first))
                        (br $loop)
                      )
                    )

                    (if (call $in_range_inclusive
                      (i32.and (local.get $mask) (i32.const 0xC0FF0000))
                      (i32.const 2160197632)
                      (i32.const 2162098176)) 
                      (then
                        (if (local.get $extend) 
                          (then
                            (i32.store (local.get $target) (local.get $mask))
                            (local.set $target (i32.add (local.get $target) (i32.const 4)))
                          )
                        )
                        (local.set $i (i32.add (local.get $i) (i32.const 4)))
                        (local.set $first (i32.const 0))
                        (br $two_byte_loop)
                      )
                    )

                    (if (local.get $extend) 
                      (then
                        (i32.store (local.get $target) (local.get $mask))
                        (local.set $target (i32.add (local.get $target) (i32.const 2)))
                      )
                    )
                    (local.set $i (i32.add (local.get $i) (i32.const 2)))
                    (br $loop)
                  )
                )

                (br $block)
              ))
          ))

        ;; i + 4 < length
        (br_if $block (i32.gt_u (i32.add (local.get $i) (i32.const 4)) (local.get $end)))

        (local.set $mask (i32.load offset=0 align=1 (local.get $i)))
        
        ;; three byte value
        (if (i32.eq (i32.and (i32.sub (local.get $mask) (i32.const 8421600)) (i32.const 12632304)) (i32.const 0)) 
          (then
            (if 
              (i32.eqz 
                (i32.or
                  (i32.eqz (i32.and (local.get $mask) (i32.const 0x200F)))
                  (i32.eqz (i32.and (i32.sub (local.get $mask) (i32.const 8205)) (i32.const 0x200F)))
                )
              )
              (then
                (if (local.get $extend) 
                  (then
                    (i32.store8 (local.get $target) (local.get $mask))
                    (local.set $target (i32.add (local.get $target) (i32.const 3)))
                  )
                )
                (local.set $i (i32.add (local.get $i) (i32.const 3)))
                (br $loop)
              ))
          )
        )

        ;; four byte value
        (if (i32.eq (i32.and (i32.sub (local.get $mask) (i32.const 2155905264)) (i32.const 3233857784)) (i32.const 0)) 
          (then
            (if (call $in_range_inclusive
              (call $rotate_r (i32.and (local.get $mask) (i32.const 0xFFFF)) (i32.const 8))
              (i32.const 4026531984)
              (i32.const 4093640847))
              (then
                (if (local.get $extend) 
                  (then
                    (i32.store8 (local.get $target) (local.get $mask))
                    (local.set $target (i32.add (local.get $target) (i32.const 4)))
                  )
                )
                (local.set $i (i32.add (local.get $i) (i32.const 4)))
                (br $loop)
              )
            )
          )
        )

        (return (i32.const -1))
      )
    )
    
    (block $block
      (loop $loop
        ;; i + 1 < length
        (br_if $block (i32.gt_u (i32.add (local.get $i) (i32.const 1)) (local.get $end)))
        
        (if (i32.lt_u (local.tee $temp (i32.load8_u (local.get $i))) (i32.const 128))
          (then
            (if (i32.eq (local.get $temp) (i32.const 92)) (then))

            (if (i32.eq (local.get $temp) (i32.const 34))
              (then
                (if
                  (i32.ge_s
                    (local.tee $temp (call $find_quote (local.get $i) (local.get $start) (local.get $i)))
                    (i32.const 0)
                  )
                  (then
                    (global.set $target (local.get $target))
                    (global.set $dq_index (local.get $temp))
                    (return (local.get $temp))
                  )
                )
              )
            )
            (if (local.get $extend) 
              (then
                (i32.store8 (local.get $target) (local.get $temp))
                (local.set $target (i32.add (local.get $target) (i32.const 1)))
              )
            )
            (local.set $i (i32.add (local.get $i) (i32.const 1)))
            (br $loop)
          )
        )

        (local.set $temp (i32.sub (local.get $temp) (i32.const 194)))

        (if (i32.lt_u (local.get $temp) (i32.const 30))
          (then
            (if (i32.ge_u (i32.add (local.get $i) (i32.const 1)) (local.get $end)) 
              (then (return (local.get $i)))
            )

            (if (i32.ge_u (i32.sub (i32.load8_u (i32.add (local.get $i) (i32.const 1))) (i32.const 128)) (i32.const 64)) 
              (then (return (i32.const -1)))
            )
            
            (if (local.get $extend) 
              (then
                (i32.store8 (local.get $target) (i32.load8_u (local.get $i)))
                (local.set $target (i32.add (local.get $target) (i32.const 1)))
                (i32.store8 (local.get $target) (i32.load8_u (i32.add (local.get $i) (i32.const 1))))
                (local.set $target (i32.add (local.get $target) (i32.const 1)))
              )
            )
            (local.set $i (i32.add (local.get $i) (i32.const 2)))
            (br $loop)
          )
        )

        (if (i32.lt_u (local.get $temp) (i32.const 46))
          (then
            (if (i32.ge_u (i32.add (local.get $i) (i32.const 2)) (local.get $end)) 
              (then (return (local.get $i))))
            
            (if 
              (i32.or 
                (i32.ge_u (i32.sub (i32.load8_u (i32.add (local.get $i) (i32.const 1))) (i32.const 128)) (i32.const 64)) 
                (i32.ge_u (i32.sub (i32.load8_u (i32.add (local.get $i) (i32.const 2))) (i32.const 128)) (i32.const 64))
              )
              (then (return (i32.const -1)))
            )

            (local.set $partialChar (i32.add 
                (i32.shl (local.get $temp) (i32.const 12))
                (i32.shl (i32.load8_u (i32.add (local.get $i) (i32.const 1))) (i32.const 6))
              )
            )

            (if 
              (i32.or 
                (i32.lt_u (local.get $partialChar) (i32.const 133120)) 
                (i32.lt_u (i32.sub (local.get $partialChar) (i32.const 186368)) (i32.const 2048))
              )
              (then (return (i32.const -1)))
            )
            
            (if (local.get $extend) 
              (then
                (i32.store8 (local.get $target) (i32.load8_u (local.get $i)))
                (local.set $target (i32.add (local.get $target) (i32.const 1)))
                (i32.store8 (local.get $target) (i32.load8_u (i32.add (local.get $i) (i32.const 1))))
                (local.set $target (i32.add (local.get $target) (i32.const 1)))
                (i32.store8 (local.get $target) (i32.load8_u (i32.add (local.get $i) (i32.const 2))))
                (local.set $target (i32.add (local.get $target) (i32.const 1)))
              )
            )

            (local.set $i (i32.add (local.get $i) (i32.const 3)))
            (br $loop)
          )
        )
      )
    )

    (if (i32.eq (local.get $i) (local.get $end))
      (then 
        (global.set $target (local.get $target))
        (return (local.get $i))
      )
    )
    (return (i32.const -1))
  )

  (func $rotate_r (param $value i32) (param $offset i32) (result i32)
    (i32.or
      (i32.shr_u (local.get $value) (local.get $offset))
      (i32.shl (local.get $value) (i32.sub (i32.const 32) (local.get $offset)))
    )
  )

  (func $in_range_inclusive (param $value i32) (param $lowerBound i32) (param $upperBound i32) (result i32)
    (i32.le_u
      (i32.sub (local.get $value) (local.get $lowerBound))
      (i32.sub (local.get $upperBound) (local.get $lowerBound))
    )
  )
)